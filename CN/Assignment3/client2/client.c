...
int main(int argc,char *argv[])
{
  ...
  while(1)
	...
	case 7:
		printf("Current Working directory in local machine is:\n");
		system("pwd");
		break;

	case 8:
		printf("Files in the current local directory : \n");
		system("ls");
		break;	

	case 9:
	  strcpy(buf, "cd ");
	  printf("Enter the path to change the local directory: ");
	  scanf("%s", buf + 3);
      if(chdir(buf+3) == 0)
	    printf("Local Directory has been changed successfully\n");
	   else
	    printf("Error in changing the Local Directory\n");
